...
NSNotificationName const NRUNetworkReachabilityChangedNotification = @"kNRUNetworkReachabilityChangedNotification";
...
@property (nonatomic, strong) NSMutableArray<NRUReachabilityHelperNotificationBlock> *notificationBlocks;
@end
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info) {
	@autoreleasepool {
		...
		[noteObject _handleReachabilityCallback];
	}
}

- (void)_handleReachabilityCallback {
	// Post a notification to notify the client that the network reachability changed.
	if (_notificationBlocks.count > 0) {
	...
	[[NSNotificationCenter defaultCenter] postNotificationName:NRUNetworkReachabilityChangedNotification
                                                        object:self];
}

- (void)_notify {
    [_notificationBlocks enumerateObjectsUsingBlock:^(NRUReachabilityHelperNotificationBlock  _Nonnull block, NSUInteger idx, BOOL * _Nonnull stop) {
        block(self);
    }];
}


- (instancetype)initWithReachabilityRef:(SCNetworkReachabilityRef)reachability
                              localWiFi:(BOOL)localwifi {
	self = [super init];
	if (nil != self) {
		self.reachabilityRef = reachability;
		self.localWiFiRef = localwifi;
        _notificationBlocks = [NSMutableArray array];
	}
	return self;
}
...
+ (NRUReachabilityHelper *) reachabilityWithHostName:(NSString *)hostName {
	...
	if (reachability != NULL) {
		retVal = [[NRUReachabilityHelper alloc] initWithReachabilityRef:reachability
                                                             localWiFi:NO];
...
+ (NRUReachabilityHelper *)reachabilityWithAddress:(const struct sockaddr_in *)hostAddress {
	SCNetworkReachabilityRef const reachability = SCNetworkReachabilityCreateWithAddress(NULL, (const struct sockaddr*)hostAddress);
	NRUReachabilityHelper *retVal = nil;
	if (reachability != NULL) {
		retVal = [[NRUReachabilityHelper alloc] initWithReachabilityRef:reachability
                                                              localWiFi:NO];
...
- (void)addNotificationBlock:(NRUReachabilityHelperNotificationBlock)block {
    [_notificationBlocks addObject:[block copy]];
}
